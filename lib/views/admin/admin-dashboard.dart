...
class _AdminDashboardState extends State<AdminDashboard> {
  // ----- Live state -----
  // We now keep numeric values so we can compute Remaining = month − expenses.
  ...
  void initState() {
    super.initState();
    // Controller is provided in main.dart; just load allocations once here as well
  ...
  Future<void> _loadDashboard() async {
    try {
      ...
      for (int i = 0; i < 7; i++) {
        ...
        final k = _dateKey(d);
        buckets[k] = 0.0;
        labels.add(wd[d.weekday - 1]); // Mon=1..Sun=7
      }
  ...
  Widget build(BuildContext context) {
    // READ SHARED CONTROLLER (single source of truth for expenses)
    final c = context.watch<OperationalExpenseController>();
    // Expenses = manual allocations + tracked (cash) for current month
    final totalExpensesThisMonth = c.totalExpensesThisMonth;
    // Remaining Funds on the dashboard = donations this month − expenses
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                      child: Text(
                        remainingStr, // <- NOW: (month donations) - (manual + tracked)
                    ...
                    // Optional: quick debug line to verify numbers
                    // Text('manual=${c.manualAllocationsTotal}, tracked=${c.trackedExpensesCash}, total=${c.totalExpensesThisMonth}', style: const TextStyle(color: Colors.white70, fontSize: 11)),
                  ],
                ),
              ),
