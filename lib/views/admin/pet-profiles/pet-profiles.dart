              ...
              switch (payload.eventType) {
                ...
                case PostgresChangeEvent.update:
                  if (newRecord != null && oldRecord != null) {
                    final index = _pets.indexWhere((p) => p.id == oldRecord['id']);
                    if (index != -1) {
                      _pets[index] = PetProfile.fromMap(newRecord);
                    }
                  }
                  break;
                case PostgresChangeEvent.delete:
                  if (oldRecord != null) {
                    _pets = _pets.where((p) => p.id != oldRecord['id']).toList();
                  }
                  break;
                default:
                  break;
            ...
            : ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _StatCard(value: '$totalPets', label: 'Total Pets'),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: _StatCard(
                            value: '$adoptionCount', label: 'For Adoption'),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: _StatCard(
                            value: '$medicalCount', label: 'Needs\nMedical Care'),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _PillButton(
                          ...
                          label: _sortNewest ? 'Sort by Newest' : 'Sort by Oldest',
                          onTap: _toggleSort,
                        ),
                      ),
                    ],
        ...
        children: [
          Text(value,
              style: const TextStyle(
                  fontWeight: FontWeight.w800, fontSize: 20, color: _brand)),
...
class _ChipOutlined extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(text,
          style: const TextStyle(
              color: _brand, fontWeight: FontWeight.w800, fontSize: 11)),
    );
  }
}
...
class _ChipDanger extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
          color: Colors.white, border: Border.all(color: _danger, width: 1.5), borderRadius: BorderRadius.circular(999)),
      child: Text(text,
          style: const TextStyle(
              color: _danger, fontWeight: FontWeight.w800, fontSize: 11)),
    );
  }
}
